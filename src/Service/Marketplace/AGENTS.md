...
- `IfoodClient` e `Food99Client` sao os unicos pontos de consulta HTTP e autenticacao dos providers nesta camada; services de capacidade nao devem chamar `HttpClientInterface` diretamente nem repetir a leitura de `OAUTH_IFOOD_CLIENT_ID` / `OAUTH_IFOOD_CLIENT_SECRET` ou `OAUTH_99FOOD_CLIENT_ID` / `OAUTH_99FOOD_CLIENT_SECRET`.
