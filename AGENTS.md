...
- Em `Food99`, o repasse semanal nao deve abater `service_fee` do payload bruto; ele fica apenas como dado do snapshot. A calibracao vigente e obtida pela reconciliacao com o portal, com `payment_processing=3.2%`, `logistics=60%` com piso de `R$ 4,50` e a comissao efetiva em torno de `8,9%`.
